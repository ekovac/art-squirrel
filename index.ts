...
parser.addArgument(['-c', '--cookies'], {
    help: 'Path to a JSON file containing the "a" and "b" auth cookies from Furaffinity.',
    required: true,
    type: String,
});
...
async function checkForId(submissionId: string|number): Promise<boolean> {
    ...
    const matchingFilenames = filenames.filter((filename) => {
        ...
        return (base == submissionId.toString() || 
...
async function main() {
    ...
    for await (const submissionListing of faClient.getFavorites('digitalfox')) {
        ...
        const jsonOutputPath = path.join(options.destination, `${submissionListing.id}.data.json`);
        ...
        await Promise.all([delay(), writeImageToDiskPromise, writeJsonToDiskPromise]).then(
            (unusedVoids) => {
                submissionsFetched++;
                if (options.veryVerbose) {
                    console.log(`Fetched submission ${submissionListing.id} (uploaded on ${submission.upload_date})`);
                }
            }
        );
    }
